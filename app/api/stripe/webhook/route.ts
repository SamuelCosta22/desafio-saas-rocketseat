  ...
  try {
    ...
    switch (event.type) {
      case "checkout.session.completed": //Pagamento realizado se status = paid
        ...
        if (metadata?.price === process.env.STRIPE_SUBCRIPTION_PRICE_ID) {
      ...
      case "checkout.session.async_payment_succeeded": //Boleto pago
        console.log(
          "Enviar um email para o usuário avisando que o boleto foi pago"
        );
        break;
      case "checkout.session.async_payment_failed": //Boleto falhou
        console.log(
          "Enviar um email para o usuário avisando que o boleto falhou"
        );
        break;
